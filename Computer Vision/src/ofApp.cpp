...
void ofApp::setup(){
    ...
    newInvertedVideoData = new unsigned char [camWidth * camHeight * 3];
    myTexture.allocate (camWidth,camHeight, GL_RGB);
    newTexture.allocate(camWidth, camHeight, GL_RGB);
...
void ofApp::draw(){
    ...
    myVideoGrabber.draw(10,ofGetHeight()/3);
    myTexture.draw(340, ofGetHeight()/3);
    newTexture.draw(680,ofGetHeight()/3);
